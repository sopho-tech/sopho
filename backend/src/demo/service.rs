...
pub async fn seed_demo_data(app_state: &AppState) {
    ...
    let sql_cells: Vec<(i32, &'static str, &'static str)> = vec![
        (1, "Monthly Revenue", "select SUM(op.payment_value) as revenue, strftime('%Y-%m', o.order_purchase_timestamp) as order_month from orders o inner join order_payments op on o.order_id = op.order_id group by order_month order by order_month asc"),
        (3, "Orders by Status", "select count(1) as order_count, order_status from orders group by order_status"),
        (5, "Avg Order Value", "select ROUND(AVG(op.payment_value),2) as avg_order_value, strftime('%Y-%m', o.order_purchase_timestamp) as order_month from orders o inner join order_payments op on o.order_id = op.order_id group by order_month"),
        (7, "Top Products", "select p.product_id, SUM(oi.price) as total_revenue from order_items oi inner join products p on oi.product_id = p.product_id group by p.product_id order by total_revenue desc limit 10"),
        (9, "Top Sellers GMV", "select seller_id, SUM(price) as gmv from order_items group by seller_id order by gmv desc limit 10"),
        (11, "Revenue by Payment Type", "select payment_type, SUM(payment_value) as total_payment_value from order_payments group by payment_type"),
        (13, "Review Score Distribution", "select review_score, count(1) as number_of_reviews from order_reviews group by review_score"),
        (15, "Review Score Trend With Time", "select ROUND(AVG(review_score), 2) as avg_review_score, strftime('%Y-%m', review_creation_date) as review_month from order_reviews group by review_month"),
        (17, "Order Reviews", "select * from order_reviews"),
    ];
    ...
    payload.layout = Some(vec![
        ...
        Layout::new(chart_id_map[&1], notebook_id, 8, 0, 4, 3),
        Layout::new(chart_id_map[&2], notebook_id, 4, 0, 4, 3),
        Layout::new(chart_id_map[&3], notebook_id, 0, 3, 4, 3),
        Layout::new(chart_id_map[&4], notebook_id, 4, 3, 4, 3),
        Layout::new(chart_id_map[&5], notebook_id, 8, 3, 4, 3),
        Layout::new(chart_id_map[&6], notebook_id, 0, 6, 6, 3),
        Layout::new(chart_id_map[&7], notebook_id, 6, 6, 6, 3),
    ]);
    ...
    info!(
        ...
        canvas_result.canvas.id, notebook_id, 17
    );
}

//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.20

pub use super::ai_configuration::Entity as AiConfiguration;
pub use super::ai_summary::Entity as AiSummary;
pub use super::canvas::Entity as Canvas;
pub use super::cell::Entity as Cell;
pub use super::configuration::Entity as Configuration;
pub use super::connection::Entity as Connection;
pub use super::conversation::Entity as Conversation;
pub use super::conversation_message::Entity as ConversationMessage;
pub use super::conversation_message_content::Entity as ConversationMessageContent;
pub use super::dashboard::Entity as Dashboard;
pub use super::notebook::Entity as Notebook;
pub use super::session::Entity as Session;
pub use super::suggested_question::Entity as SuggestedQuestion;
pub use super::user::Entity as User;
